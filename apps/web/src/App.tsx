...
import { Trash2, Send, Loader2, RotateCw } from 'lucide-react';
import { sendChat, getSettings, updateSettings, getHistory, deleteHistoryMessage, getWeather, type WeatherData } from './api/client';
...
function App() {
  ...
  useEffect(() => {
    if (!token) return;
    const fetchWeather = () => getWeather(token).then(setWeather).catch(console.error);
    fetchWeather();
    ...
    return (
      <div className="login-container">
        <form className="login-box" onSubmit={handleLogin}>
          <img src="/assets/img/header.jpg" alt="Logo" className="login-logo" />
          <input
            type="email"
    ...
    <div className="app-layout">
      <Sidebar 
        ...
        weather={weather ? {
          ...
          icon: weather.conditionCode < 3 ? 'sun' : weather.conditionCode < 50 ? 'cloud' : 'cloud-rain'
        } : null}
      />
      ...
      <main className="chat-main">
        {currentPage === 'inicio' ? (
            ...
            <header className="chat-header">
              <div className="chat-header-main">
                <div className="chat-header-brand" onClick={() => handleRefresh()}>
                  <h1 className="header-title-desktop">Chat con Kairo</h1>
                  <img src="/assets/img/header.jpg" alt="Logo" className="header-logo-mobile" />
                </div>
          ...
          activeCollection && <CollectionPage collection={activeCollection} token={token!} />
        )}
      </main>
    </div>
  );
